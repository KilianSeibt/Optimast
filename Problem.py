...
class Problem:
    ...
    def solve(self) -> tuple[set[Tower], set[Tower]]:
        ...
        """
        Solves the problem using the MILP solver PuLP
        So far, No penalty is implemented
        :return: coordinates for the small and the large towers
        """
        ...
        overcoverage = {}
        for city in self.cities:
            overcoverage[city] = LpVariable(
                f"overcoverage_{city.name}",
                lowBound=0,
                cat="Integer"
            )

        penalty_weight = 0.1  # Beispielwert
